...
public class Maxsubsqence
  {
   static int maxsubseque=0;
    public static void solve(int idx,ArrayList<Integer> arr,ArrayList<Integer> tmp,int k){
          if(idx==arr.size())
          {
            if(maxsubseque<tmp.size())
              {
                maxsubseque=tmp.size();
              }
            return;
         }
        if (tmp.size()==0 || (arr.get(idx)-tmp.get(tmp.size()-1))%k==0)//2 condition for pick..                
          {
             tmp.add(arr.get(idx));
        ...
        int idx=0;
        solve(idx,arr,tmp,k);
        System.out.println("MaxSubsSequence :"+maxsubseque);
    }
}
